...
exports.listing = function (req, res) {
	...
	Customer.find({
		...
		.select(['_id', 'name', 'mail', 'address','mobile','gender','profileImage','state','city','country', 'status', 'heard_about', 'created_date','userType','social_provider','login_time'])
		.sort({ 'created_date': -1 })
...
exports.update = function (req, res) {
	profileImageUpload(req, res, function (err) {
		...
		Customer.update({ _id: req.body._id }, req.body, function (err, updatedUser) {
			if (err) {
				req.flash('success', 'Customer details updated!');
				res.redirect(baseUrl + 'admin/customer');
			} else {
				req.flash('success', 'Customer details updated!');
				res.redirect(baseUrl + 'admin/customer');
	...
	if(req.params.status == "active"){
		...
		Customer.update({ _id: req.params.id }, req.body, function (err, updatedresult) {
			if (err) {
				...
				req.flash('success', `Customer ${statusMessage} Successfully.`);
				res.redirect(baseUrl + 'admin/customer');
			}
		});
	}
		...
		var appointmentList = []
		appointments.find({ customer : userId, status: { $in: ["Upcoming", "Ongoing"] } }, function(err , result){
			if(err){
				req.flash('error', 'Sorry something went wrong.');
				res.redirect(baseUrl + 'admin/customer');
			} else {
				...
				if(appointmentList.length > 0){
					req.flash('error', 'Customer should be not inactive due to pending appointments.');
					res.redirect(baseUrl + 'admin/customer');
				} else {
					Customer.update({ _id: req.params.id }, req.body, function (err, updatedresult) {
						if (err) {
...
exports.exportToCsv = function (req, res) {
	...
	Customer.find({'role_id': '2', 'status': { $in: ['inactive','active'] }})
		.sort({ 'created_date': -1 })
		.exec(async function (err, result) {
			if (err) {
				...
				for (var i = 0; i < result.length; i++) {
					...
					dataArray.push({ "S. No.": i+1, "Name": result[i].name, "Email": result[i].mail, "Gender": result[i].gender, "Address": result[i].address, "Mobile": result[i].mobile,"Hear About Us":result[i].heard_about,  "Last Active On": result[i].login_time ? dateFormat(result[i].login_time, "mm-dd-yyyy h:MM TT") : created_date, "User Type":result[i].userType, "Social Provider":result[i].social_provider,  "Created Date": created_date, "Status": result[i].status });
				}
...
exports.delete = function (req, res) {
	...
	appointments.find({ customer : userId, status: { $in: ["Upcoming", "Ongoing"] } }, function(err , result){
		if(err){
			...
			appointmentList = result;
			if(appointmentList.length > 0){
				req.flash('error', 'Customer cannot be deleted due to pending appointment(s).');
				res.redirect(baseUrl + 'admin/customer');
			} else {
				Customer.update({ _id: req.params.id }, req.body, function (err, updatedresult) {
					if (err) {
						...
						appointments.deleteMany({mail: userEmail}, async function(err, removeAppointment) {
							if(err){
								return done(err);
							} else {
								await sendReminder.deleteMany({mail: userEmail});
								await paymentReminder.deleteMany({cusMail: userEmail});
								await connectedList.deleteMany({cusMail: userEmail});
...
exports.datefilter = function (req, res) {
	...
	Customer.find({
	    ...
	    .select(['_id', 'name', 'mail', 'address','profileImage','state','city','country', 'status', 'heard_about', 'created_date','userType','social_provider','login_time'])
		.sort({ 'created_date': -1 })
		.exec(function (err, result) {
			if (err) {
				data.result = '';
